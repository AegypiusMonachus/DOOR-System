...
def load_Excel(path):
    ...
    for c in range(sheet.ncols):
        ...
        if (sheet.cell(col_title_row,c).ctype > 0): # 单元格为string类型 0 empty,1 string, 2 number, 3 date, 4 boolean, 5 error
            ...
            # elif sheet.cell_value(col_title_row,c) == ("订单号"):
            #         exported_produce = c
            #         col_name_index_map["exported_produce"] = c
            # elif sheet.cell_value(col_title_row,c) == ("订单批次"):
            #         exported_produce = c
            #         col_name_index_map["exported_produce"] = c

    #print("col_name_index_map", col_name_index_map)
    '''

    for c in range(sheet.ncols):
        #print(sheet.cell_value(0,c))
        if (sheet.cell(0,c).ctype > 0): # 单元格为string类型 0 empty,1 string, 2 number, 3 date, 4 boolean, 5 error
            if sheet.cell_value(0,c) == ("派出所"):
                    pcs = c
                    col_name_index_map["pcs"] = c
            elif sheet.cell_value(0,c) == ("街路巷"):
                    street = c
                    col_name_index_map["street"] = c
            elif sheet.cell_value(0,c) == ("门牌号"):
                    dp_num = c
                    col_name_index_map["dp_num"] = c
            elif sheet.cell_value(0,c) == ("门牌名称"):
                    dp_name = c
                    col_name_index_map["dp_name"] = c
            elif sheet.cell_value(0,c) == ("全球唯一码带门牌名称"):
                    global_id_with_dp_name = c
                    col_name_index_map["global_id_with_dp_name"] = c
            elif sheet.cell_value(0,c) == ("门牌类型"):
                    dp_type = c
                    col_name_index_map["dp_type"] = c
            elif sheet.cell_value(0,c) == ("门牌规格"):
                    dp_size = c
                    col_name_index_map["dp_size"] = c
            elif sheet.cell_value(0,c) == ("门牌id"):
                    dp_id = c
                    col_name_index_map["dp_id"] = c
            elif sheet.cell_value(0,c) == ("门牌号纯数字"):
                    dp_num_trans = c
                    col_name_index_map["dp_num_trans"] = c
    '''
    temp_data_len = 0
...
def export_excel(datas_list, col_name_index_map, path="", filename="", **kw):
    ...
    print("need_order_batch", kw.get("need_order_batch"))
    print("need_order_id", kw.get("need_order_id"))
    ...
    biaoti = ["序号","地址名称","规格","是否上墙","安装日期","确认安装签名","门牌id","全球唯一码"]
    ...
    for pcs in pcs_map.keys():
        ...
        if w.find("派出所") >= 0 or w.find("分局") >= 0:
            ...
            for type in street_dp_type_dp_size_map.keys():
                ...
                # if w.find("佛山市南海区") >= 0:
                #     # data_available = 1
                #     sheetname = w[w.index("派出所"):]
                #     sum_for_community = 0
                #     for size in street_dp_type_dp_size_map[type].keys():
                #         for street in street_dp_type_dp_size_map[type][size].keys():
                #             sum_for_community += len(street_dp_type_dp_size_map[type][size][street])
                #     sheetname = sheetname[3:] + str(sum_for_community)
                #
                #
                #
                ...
                # 第一行，标题
                title = "广州市伟圣实业有限公司"
                worksheet.write_merge(wirte_now_row, wirte_now_row, 0, 5, title, OneStyle)
                wirte_now_row += 1
                # 第二行 子标题
                sub_title = "二维码" + type + "安装确认表"
                worksheet.write_merge(wirte_now_row, wirte_now_row, 0, 5, sub_title, OneStyle)
                wirte_now_row += 1
                # 第三行 项目名称，送货单号
                try:
                ...
                deliver_id = "送货单号："
                worksheet.write(wirte_now_row, 4, deliver_id, ThreeStyle)
                wirte_now_row += 1
                ...
                worksheet.write(wirte_now_row, 4, produce_id, ThreeStyle)
                wirte_now_row += 1
                order_batch = ""
                if kw.get("need_order_batch"):
                    if col_name_index_map.get("order_batch"):
                ...
                worksheet.write(wirte_now_row, 4, list_id, ThreeStyle)
                wirte_now_row += 1
                ...
                for size in sorted(list(street_dp_type_dp_size_map[type].keys()), reverse=True):
                    ...
                    for street in dp_sort.sort_for_1D(list(street_dp_type_dp_size_map[type][size].keys())):
                        ...
                        temp_sum_for_static += len(street_dp_type_dp_size_map[type][size][street])
                        ...
                        for data_for_street in dp_sort.sort_by_dp_and_street(list(street_dp_type_dp_size_map[type][size][street]), col_name_index_map['dp_name'],
                            ...
                            if not col_width:
                                ...
                                col_width.append(len_byte(data_for_street[col_name_index_map['dp_size']]))
                                col_width.append(10)
                                col_width.append(20)
                                col_width.append(20)
                                ...
                                if col_width[1] <= len_byte(data_for_street[col_name_index_map['dp_name']])*1.5:
                                    col_width[1] = len_byte(data_for_street[col_name_index_map['dp_name']])*1.5


                            # 写序号
                            worksheet.write(wirte_now_row, 0, temp_sum-len(street_dp_type_dp_size_map[type][size][street])+data_for_street_sum+1,
                                            content_Style)
                            data_for_street_sum += 1
                            ...
                            if data_for_street[col_name_index_map['dp_name']] == '':
                                ...
                                if col_name_index_map.get('exported_produce') :
                                    ...
                                    if int(data_for_street[col_name_index_map['exported_produce']]) >= 1:
                                        worksheet.write(wirte_now_row, 1, data_for_street[col_name_index_map['dp_name']],
                            ...
                            # 写规格
                            worksheet.write(wirte_now_row, 2, size,
                                            content_Style)
                            ...
                            # 写安装日期
                            worksheet.write(wirte_now_row, 4, "",
                                            content_Style)

                            # 写确认安装签名
                            worksheet.write(wirte_now_row, 5, "",
                                            content_Style)
                    ...
                    statistics_datas[sheetname][size] = temp_sum_for_static
                ...
                worksheet.write(wirte_now_row, 4, "", content_Style)
                worksheet.write(wirte_now_row, 5, "", content_Style)
                wirte_now_row += 1

                worksheet.write(wirte_now_row, 5, "", content_right_border)
                wirte_now_row += 1

                worksheet.write(wirte_now_row, 0, "安装情况：以上门牌已安装合格", ThreeStyle)
                worksheet.write(wirte_now_row, 5, "", content_right_border)
                wirte_now_row += 1
                worksheet.write(wirte_now_row, 5, "", content_right_border)
                wirte_now_row += 1
                ...
                worksheet.write_merge(wirte_now_row, wirte_now_row, 3, 4, "派出所单位签字盖章：", TwoStyle)
                worksheet.write(wirte_now_row, 5, "", content_right_border)
                wirte_now_row += 1
                worksheet.write_merge(wirte_now_row, wirte_now_row, 3, 4, "日期：", TwoStyle)
                worksheet.write(wirte_now_row, 5, "", content_right_border)
                wirte_now_row += 1
                worksheet.write(wirte_now_row, 5, "", content_right_border)
                wirte_now_row += 1
                ...
                worksheet.write(wirte_now_row, 5, "", content_bottom_right_border)
                wirte_now_row += 1
    ...
    # 最后保存
    save_path = os.path.dirname(path) + "/received_" + str(filename_pre) + '_' + common.format_ymdhms_time_now_for_filename() +  ".xls"
