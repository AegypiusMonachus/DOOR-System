...
def export_excel(datas_list, col_name_index_map, path="", filename="", **kw):
    ...
    for pcs in pcs_map.keys():
        ...
        if w.find("派出所") >= 0 or w.find("分局") >= 0:
            ...
            for type in street_dp_type_dp_size_map.keys():
                ...
                # 第一行，标题
                worksheet.write_merge(wirte_now_row, 0, 0, 3, Tou, OneStyle)
                wirte_now_row += 1
                # 第二行
                worksheet.write_merge(wirte_now_row, 1, 0, 3, b, TwoStyle)
                wirte_now_row += 1

                # 第三行,单位名
                worksheet.write_merge(wirte_now_row, 2, 0, 3, danwei, ThreeStyle)
                wirte_now_row += 1
                ...
                for size in sorted(list(street_dp_type_dp_size_map[type].keys()), reverse=True):
                    ...
                    wirte_now_row += 1
                    worksheet.write(wirte_now_row, 0, "")
                    wirte_now_row += 1
    ...
    # 最后保存
    save_path = os.path.dirname(path) + "/produced_" + str(filename_pre) + '_' + common.format_ymdhms_time_now_for_filename() +  ".xls"
...
def export(path, filename="", **kw):
    ...
    待更新，佛山生成生产单
    '''
    # if col_name_index_map.get(""):
    #     temp_datas = ["(\"" + "\",\"".join(
    #         i + other_datas + [chinese_to_arabic_with_connection(i[title_col_map["dp_num"]])] + [
    #             i[title_col_map["global_id"]] + i[title_col_map["dp_name"]]]) + "\")" for i in datas[head:tail]]
